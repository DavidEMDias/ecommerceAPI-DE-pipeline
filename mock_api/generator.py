...
# Every time you run this program, you’ll get the exact same outputs
random.seed(42) 
...
class DataStore:
    ...
    def _generate(self): # _ means function intended for internal use
        ...
        for i in range(1000):
            ...
            self.customers.append({
                ...
                "is_churned":churn # Stopped using the service
            })
        ...
        for i in range(10000):
            ...
            }[c["industry"]])[0] # Each industry has its own probability distribution over products.
                                 # Example: E-commerce: 40% Basic, 35% Pro, 5% Enterprise, 10% Analytics, 10% Support
                                 # Pick one product according to the weights.

            base=PRICES[product] # Base Price

            seat=1 # 1 seat (single user license)
            if product in ["Pro","Enterprise"] and random.random()<0.25:
                seat=random.choices([2,3,4,5],[0.4,0.3,0.2,0.1])[0] # For Pro/Enterprise (team plans) → 25% chance of having 2–5 seats

            amount=base*seat # Total payment = unit price × number of seats

            created=_rand_date(180)
            status=random.choices(STATUSES,STAT_W)[0]

            fee=round(amount*0.029+0.3,2) # Stripe-style processing fee: 2.9% + $0.30 per transaction

            refunded=0.0
            if status=="refunded":
                refunded=round(amount*random.uniform(0.25,1.0),2) # If the payment is refunded, simulate partial or full refund.
            ...
            self.payments.append({
                ...
                "product":product,
                "amount":float(amount),
                "currency":"USD" if c["country"] not in EUR_COUNTRIES else "EUR",
                "status":status,
        ...
        for i in range(30000):
            ...
            if dt.weekday() in (5,6) and random.random()<0.2:
                continue # Reduce weekend traffic by skipping 20% of weekend sessions

            cust=None # Starts as Anonymous
            if random.random()<0.35:
                cust=random.choice(self.customers)["customer_id"] # around 35% of sessions to be an existing customer
            
            source=random.choices(SOURCES,SRC_W)[0]
            medium=random.choices(MEDIUMS,MED_W)[0]
            device=random.choices(DEVICES,DEV_W)[0]
            pageviews=max(1,int(random.expovariate(1/2))) # Most sessions will have 1 to 2 pages, min 1 page.
            duration=int(random.gammavariate(2,45)) # Many short visits, some medium ones, a few long sessions, average session lasts 90 seconds
            bounced = 1 if (pageviews == 1 and duration < 10) else 0 # boolean
            
            conv_prob = (0.02 
                    + (0.02 if medium=="cpc" else 0) # example: emails +3%, cpc +2% convertion boost
                    + (0.03 if medium=="email" else 0)
                    + (0.01 if device=="desktop" else 0) 
                    + (0.01 if pageviews>=3 else 0)
                    ) 
            converted = 1 if conv_prob > random.random() else 0 # In this case - a purchase
